...
%   Input:
%       mask: original mask
%       segmentation_para.watershed_para: Parameters for watershed
%   Output:
...
curr_basins = mask;
num_iteration = segmentation_para.nuc_radius * 2;
for i=1:num_iteration
    % erode image once
    curr_basins = imerode(curr_basins, strel('disk', 1, 0));
    
    % find components with area smaller than the threshold
    after_removal = bwareaopen(curr_basins, segmentation_para.watershed_para.max_thresh_component_size);
    id = bwareaopen(curr_basins, segmentation_para.watershed_para.min_thresh_component_size) & xor(curr_basins, after_removal);
...
num_iteration = round(segmentation_para.nuc_radius / 3);
for i=1:num_iteration
