...
//      all_morphology_posterior_stepscore: output from classification algorithm, converted to score
...
//          c++: a vector
//      all_morphology_posterior_stepscore
...
//
int generate_tracks(std::vector<int>& all_num_detections, std::vector<std::vector<std::vector<double> > >& all_morphology_posterior_stepscore, std::vector<std::vector<std::vector<Migration_Link> > >& all_stepscore_migration, std::vector<std::vector<double> >& all_stepscore_inout_frame, Track_Para& track_para, std::vector<Track>& all_tracks) {
    ...
    while (1) {
        ...
        // Frame 1
        {
            ...
            for (int i=0; i<num_detections; ++i) {
                ...
                if (prev_cellcount <= 2) { // note that we only train 0, 1, 2 cells per detection
                    new_frame.current_score[i] = all_morphology_posterior_stepscore[0][i][prev_cellcount];
        ...
        for (int i=1; i<num_frames; ++i) {
            ...
            for (int j=0; j<previous_frame_num_detections; ++j) {
                double temp = all_morphology_posterior_stepscore[i-1][j][5]*(!recorded_if_apoptosis[i-1][j]);
                temp = adjust_stepscore(temp, track_para);
            ...
            for (int j=0; j<num_detections; ++j) {
                ...
                if (prev_cellcount <= 2) { // can add one cell
                    current_score_adding_cell = all_morphology_posterior_stepscore[i][j][prev_cellcount];
                } else { // both previous and current are at least 3 cells per detection -> 0
                ...
                for (int i_link=0; i_link<num_migration_links; ++i_link) {
                    ...
                    for (int i_track=0; i_track<num_tracks; ++i_track) {
                        ...
                        double temp = all_morphology_posterior_stepscore[i-1][track_previous_id][3]  /* mitosis score */
                            + all_morphology_posterior_stepscore[i][track_current_id][4] /* mitosis exit score */
                            + all_morphology_posterior_stepscore[i][j][4] /* mitosis exit score */
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    ...
    Track_Para track_para;
    {
        ...
        val = mxGetPr(mxGetField(prhs[4], 0, "max_num_tracks")); 
        if (mxIsInf(*val)) {
            track_para.max_num_tracks = Inf_int;
        } else {
            track_para.max_num_tracks = (int)*val;
        }
        ...
        val = mxGetPr(mxGetField(prhs[4], 0, "max_recorded_link"));
        if (mxIsInf(*val)) {
            track_para.max_recorded_link = Inf_int;
        } else {
            track_para.max_recorded_link = (int)*val;
        }
    ...
    // all_morphology_posterior_stepscore
    std::vector<std::vector<std::vector<double> > > all_morphology_posterior_stepscore;
    {
        ...
        for (mwIndex i_frame=0; i_frame<frame_dims[0]; ++i_frame) {
            ...
            all_morphology_posterior_stepscore.push_back(detection_by_feature_vector);
        }
    }
    ...
    std::vector<std::vector<std::vector<Migration_Link> > > all_stepscore_migration;
    {
        ...
        for (mwIndex i_frame=0; i_frame<frame_dims[0]; ++i_frame) {
            ...
            for (mwIndex i_detection=0; i_detection<detection_by_gap_dims[0]; ++i_detection) {
                ...
                int num_links_to_save = std::min((int)source_score.size(), track_para.max_recorded_link);
    ...
    if (track_para.max_num_tracks == Inf_int) {
        all_tracks.reserve(1000);
    } else {
        all_tracks.reserve(track_para.max_num_tracks+1);
    }
    generate_tracks(all_num_detections, all_morphology_posterior_stepscore, all_stepscore_migration, all_stepscore_inout_frame, track_para, all_tracks);
