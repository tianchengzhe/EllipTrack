...
switch (length(available_labels))
    case 0
        warning('No training data for cell counts per ellipse. Assume equal probability for 3 labels.');
        for i=1:num_frames
            all_cell_count_prob{i}(:) = 1/3;
        end
    case 1  % only 1 label exists, no need to learn anything
        for i=1:num_frames
            warning('Only training data for one cell count label is provided. Assume empty_prob for missing labels.');
            all_cell_count_prob{i}(:, missing_labels) = track_para.empty_prob;
            all_cell_count_prob{i}(:, available_labels) = 1-2*track_para.empty_prob;
        end
    case 2  % can train an SVM classifier for 2 labels
        % only select those training data for cell numbers
        warning('Only training data for two cell count labels is provided. Assume empty_prob for missing labels.');
        ...
        for i=1:num_frames
            ...
            all_cell_count_prob{i}(:, missing_labels) = track_para.empty_prob;
            all_cell_count_prob{i}(:, available_labels(1)) = (1-track_para.empty_prob)*temp(:,2);
            all_cell_count_prob{i}(:, available_labels(2)) = (1-track_para.empty_prob)*(1-temp(:,2));
        end
