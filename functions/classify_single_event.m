...
%       track_para (varargin{5}): Parameters for tracking
%   Output
%       all_event_prob: Probability of an ellipse to display the event.

% extract info to use
if (nargin < 4)
    error('Wrong number of inputs for classify_single_event.');
end
...
if (sum(adjusted_labels) == 0) % no training data, give empty_prob
    for i=1:num_frames
        all_event_prob{i}(:) = track_para.empty_prob;
    end
