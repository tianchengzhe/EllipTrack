...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] nuc_raw_image_path: Path to the folder containing images of\n");
fprintf(fileID, "%% the nuclear channel.\n");
fprintf(fileID, "%% [Essential] nd2_frame_range: Range of frames each ND2 file stores.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] cmosoffset_path: Path to the MAT file storing the camera dark\n");
fprintf(fileID, "%% noises (CMOS Offset).\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] if_global_correction: Indicator variable of whether to perform\n");
fprintf(fileID, "%% global jitter correction.\n");
fprintf(fileID, "if_global_correction = %g;\n", handles.curr_parameters.global_setting.if_global_correction);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] output_path: Path to the folder storing the outputs.\n");
...
fprintf(fileID, "%% [Optional] if_active_contour: Indicator variable of whether to run Active\n");
fprintf(fileID, "%% Contour.\n");
fprintf(fileID, "if_active_contour = %g;\n", handles.curr_parameters.segmentation_para.if_active_contour);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] if_watershed: Indicator variable of whether to run Watershed.\n");
fprintf(fileID, "if_watershed = %g;\n", handles.curr_parameters.segmentation_para.if_watershed);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] if_seg_correction: Indicator variable of whether to run\n");
fprintf(fileID, "%% Correction with Training Datasets.\n");
fprintf(fileID, "if_seg_correction = %g;\n", handles.curr_parameters.segmentation_para.if_seg_correction);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential]: if_print_mask: Indicator variable of whether to generate the\n");
fprintf(fileID, "%% mask before Ellipse Fitting.\n");
fprintf(fileID, "%% [Essential]: mask_path: Path to the folder storing the masks.\n");
fprintf(fileID, "if_print_mask = %g;\n", handles.curr_parameters.segmentation_para.if_print_mask);
fprintf(fileID, "mask_path = '%s';\n", handles.curr_parameters.segmentation_para.mask_path);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] if_print_ellipse_movie: Indicator variable of whether to\n");
fprintf(fileID, "%% generate 'Ellipse Movie' where fitted ellipses are overlaid on the\n");
fprintf(fileID, "%% nuclear images.\n");
fprintf(fileID, "%% [Essential] ellipse_movie_path: Path to the folder storing the 'Ellipse\n");
fprintf(fileID, "%% Movies'.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] if_save_seg_info: Indicator variable of whether to save the\n");
fprintf(fileID, "%% features of ellipses in each frame ('seg info').\n");
fprintf(fileID, "%% [Essential] seg_info_path: Path to the folder storing the 'seg info'.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] nuc_radius: Average radius (in pixels) of a nucleus.\n");
fprintf(fileID, "nuc_radius = %g;\n", handles.curr_parameters.segmentation_para.nuc_radius);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] max_hole_size_for_fill: Maximal area (in pixels) of a hole to\n");
fprintf(fileID, "%% fill in.\n");
fprintf(fileID, "max_hole_size_for_fill = %g;\n", handles.curr_parameters.segmentation_para.max_hole_size_for_fill);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] min_component_size_for_nucleus: Minimal area (in pixels) of a\n");
fprintf(fileID, "%% component to contain a nucleus.\n");
...
fprintf(fileID, "%% [Optional] blurradius: Both methods. Radius (in pixels) of disk for\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] if_log: Both methods. Indicator variable of whether to\n");
fprintf(fileID, "%% log-transform the images.\n");
fprintf(fileID, "if_log = %g;\n", handles.curr_parameters.segmentation_para.image_binarization_para.if_log);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] if_blob_detection: Both methods. Indicator variable of\n");
fprintf(fileID, "%% whether to run Blob Detection.\n");
...
fprintf(fileID, "%% [Optional] blurradius: Radius (in pixels) of disk for smoothing.\n");
fprintf(fileID, "blurradius = %g;\n", handles.curr_parameters.segmentation_para.active_contour_para.blurradius);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] if_log: Indicator variable of whether to log-transform the\n");
fprintf(fileID, "%% images.\n");
fprintf(fileID, "if_log = %g;\n", handles.curr_parameters.segmentation_para.active_contour_para.if_log);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] if_global: Indicator variable of whether to run the Global\n");
...
fprintf(fileID, "%% [Optional] max_thresh_component_size: Maximal area (in pixels) of a peak.\n");
fprintf(fileID, "%% [Optional] min_thresh_component_size: Minimal area (in pixels) of a peak.\n");
...
fprintf(fileID, "%% Parameters in Zafari et al 2015. Refer to the original publication for\n");
fprintf(fileID, "%% more information.\n");
...
fprintf(fileID, "%% [Essential] training_data_path: Paths to the training datasets.\n");
...
fprintf(fileID, "%% ellipse for splitting.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] min_no_cell_prob: Minimal probability (between 0 and 1) of\n");
fprintf(fileID, "%% containing no nucleus for removal.\n");
fprintf(fileID, "min_no_cell_prob = %g;\n", handles.curr_parameters.segmentation_para.seg_correction_para.min_no_cell_prob);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] min_two_cells_prob: Minimal probability (between 0 and 1) of\n");
fprintf(fileID, "%% containing two nuclei for splitting.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% Part 1. Predict Probabilities\n");
fprintf(fileID, "%% [Essential] training_data_path: Paths to the training datasets.\n");
...
fprintf(fileID, "%% DO NOT CHANGE THE SCRIPTS ABOVE\n");
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] empty_prob: Probability of an event (between 0 and 1) if no\n");
fprintf(fileID, "%% training samples are provided.\n");
fprintf(fileID, "empty_prob = %g;\n", handles.curr_parameters.track_para.empty_prob);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Important] if_switch_off_before_mitosis: Indicator variable of whether\n");
fprintf(fileID, "%% to ignore the probability of being an mitotic cell (Before M) during\n");
fprintf(fileID, "%% mitosis detection.\n");
fprintf(fileID, "%% [Optional] if_switch_off_after_mitosis: Indicator variable of whether to\n");
fprintf(fileID, "%% ignore the probability of being a newly born cell (After M) during\n");
fprintf(fileID, "%% mitosis detection.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] if_similarity_for_migration: Indicator variable of whether to\n");
fprintf(fileID, "%% consider ellipse similarity when computing migration probabilities.\n");
fprintf(fileID, "if_similarity_for_migration = %g;\n", handles.curr_parameters.track_para.if_similarity_for_migration);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Important] migration_sigma: Standard deviation (in pixels) of random\n");
fprintf(fileID, "%% walk in one frame and one direction. If NaN is chosen, the value will be\n");
fprintf(fileID, "%% inferred from the training datasets.\n");
fprintf(fileID, "migration_sigma = %g;\n", handles.curr_parameters.track_para.migration_sigma);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] max_migration_distance_fold: Maximal distances (expressed as\n");
fprintf(fileID, "%% folds of migration_sigma) an ellipse can travel in one frame and one\n");
fprintf(fileID, "%% direction.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] max_gap: Maximal gap (in frames) of migration. Gap = number of\n");
fprintf(fileID, "%% frames to skip + 1.\n");
fprintf(fileID, "max_gap = %g;\n", handles.curr_parameters.track_para.max_gap);
fprintf(fileID, "\n");
fprintf(fileID, "%% Part 2. Construct Cell Tracks\n");
fprintf(fileID, "%% [Optional] skip_penalty: Penalty score for skipping one frame.\n");
fprintf(fileID, "skip_penalty = %g;\n", handles.curr_parameters.track_para.skip_penalty);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] multiple_cells_penalty: Penalty score for two cells\n");
...
fprintf(fileID, "%% mitotic and newly born cells for mitosis detection.\n");
fprintf(fileID, "min_mitosis_prob = %g;\n", handles.curr_parameters.track_para.min_mitosis_prob);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] max_num_tracks: Maximal number of tracks to construct.\n");
fprintf(fileID, "max_num_tracks = %g;\n", handles.curr_parameters.track_para.max_num_tracks);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] min_track_score: Minimal score of a track.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] max_recorded_link: For each ellipse, maximal number of\n");
fprintf(fileID, "%% migration events to keep.\n");
fprintf(fileID, "max_recorded_link = %g;\n", handles.curr_parameters.track_para.max_recorded_link);
fprintf(fileID, "\n");
fprintf(fileID, "%% Part 3. Post-Processing\n");
fprintf(fileID, "%% [Optional] min_swap_score: Minimal score for swapping cell tracks.\n");
fprintf(fileID, "min_swap_score = %g;\n", handles.curr_parameters.track_para.min_swap_score);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] fixation_min_prob_before_mitosis: Minimal probability of\n");
fprintf(fileID, "%% mitotic cells (Before M) for mitosis detection.\n");
fprintf(fileID, "%% [Optional] fixation_min_prob_after_mitosis: Minimal probability of newly\n");
fprintf(fileID, "%% born cells (After M) for mitosis detection.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] min_track_length: Minimal length (in frames) of a valid track.\n");
fprintf(fileID, "min_track_length = %g;\n", handles.curr_parameters.track_para.min_track_length);
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] max_num_frames_to_skip: Maximal number of frames a valid track\n");
fprintf(fileID, "%% can skip.\n");
fprintf(fileID, "max_num_frames_to_skip = %g;\n", handles.curr_parameters.track_para.max_num_frames_to_skip);
fprintf(fileID, "\n");
fprintf(fileID, "%% Part 4. Visualization\n");
fprintf(fileID, "%% [Essential] if_print_vistrack: Indicator variable of whether to generate\n");
fprintf(fileID, "%% 'Vistrack Movies' where fitted ellipses are overlaid on the nuclear\n");
fprintf(fileID, "%% images and Cell Track IDs are displayed next to the ellipses they mapped\n");
fprintf(fileID, "%% to.\n");
fprintf(fileID, "%% [Essential] vistrack_path: Path to the folder storing 'Vistrack Movies'.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% Part 1. Signal Channels\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] additional_raw_image_paths: Paths to the folder containing\n");
...
fprintf(fileID, "%% DO NOT CHANGE THE SCRIPTS ABOVE\n");
fprintf(fileID, "\n");
fprintf(fileID, "%% [Essential] additional_bias_paths: Paths to the MAT files storing the\n");
fprintf(fileID, "%% illumination bias of the signal channels.\n");
...
fprintf(fileID, "end\n");
fprintf(fileID, "%% DO NOT CHANGE THE SCRIPTS ABOVE\n");
fprintf(fileID, "%% [Essential] if_compute_cyto_ring: Indicator variables of whether to\n");
fprintf(fileID, "%% extract the signals in the cytoplasmic ring.\n");
...
fprintf(fileID, "\n");
fprintf(fileID, "%% [Optional] foreground_dilation_size: Minimal distance (in pixels) between\n");
fprintf(fileID, "%% the image background and a nucleus.\n");
